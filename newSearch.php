...
//$each="";
				...
				$terms = explode(" ",$k);
				$query .= "SELECT distinct document.path FROM keyword INNER JOIN key_specifier on 
					keyword.idKeyword= key_specifier.Keyword_idKeyword INNER join document on 
					document.idDocument = key_specifier.docid where ";
				...
				foreach ($terms as $each)
				{
					$i++;
					if ($i == 1)
					$query .= "Keyword_name LIKE '%$each%' ";
					
					else
					$query .= " OR Keyword_name LIKE '%$each%' ";	
				}
				$query .= "ORDER BY Keyword_name LIMIT 0, 5" ;
		#echo($query);
